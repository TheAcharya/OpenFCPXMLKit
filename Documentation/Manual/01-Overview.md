...
| **ShotExtractor** | Primary-timeline stills → PNG + CSV / Notion JSON (`extractShots` / `planShots`) |
...
**Layer stack (bottom → top):** XML → Parsing → Model → Extraction → **Projection** → Reporting. **Shot Extraction** is a parallel Projection consumer (independent of Reporting). Extend lower layers before adding report-only XML walks. See [12 — Timeline Projection](12-Timeline-Projection.md), [21 — Shot Extraction](21-Shot-Extraction.md), [ARCHITECTURE.md](../../ARCHITECTURE.md) §2.7, and [GUARDRAILS.md](../../GUARDRAILS.md) for must / must-not constraints.
