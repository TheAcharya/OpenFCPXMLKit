...
@Suite("Effects report policy")
struct FCPXMLEffectsReportPolicyTests {
    ...
    @Test("Non-filter video effects are always included")
    func shouldIncludeNonFilterVideoEffectsAlwaysTrue() throws {
        let host = try requireExtractedHost(from: parseInlineFCPXML(minimalTimeline()), elementName: "asset-clip")
        ...
        for kind in kinds {
            ...
            #expect(EffectsReportPolicy.shouldInclude(effect), "Expected \(kind) to be included")
        }
    }

    @Test("Filter-video effects are included by default")
    func shouldIncludeFilterVideoByDefault() throws {
        let host = try requireExtractedHost(from: parseInlineFCPXML(minimalTimeline()), elementName: "asset-clip")

        for name in ["Custom Vendor Filter", "Cross Dissolve", "Timecode", "Shape Mask"] {
            let effect = makeExtractedEffect(name: name, host: host)
            #expect(EffectsReportPolicy.shouldInclude(effect), "Expected \(name) to be included")
        }
    }

    @Test("Filter-video is excluded when host is occluded")
    func shouldExcludeFilterVideoWhenHostIsOccluded() async throws {
        ...
        #expect(!EffectsReportPolicy.shouldInclude(effect))
    }

    @Test("Extracted effects are filtered by policy on timeline")
    func extractedEffectsFilteredByPolicyOnTimeline() async throws {
        ...
        let volume = try #require(effects.first { $0.kind == .volume })

        #expect(EffectsReportPolicy.shouldInclude(crossDissolve))
        #expect(EffectsReportPolicy.shouldInclude(volume))
    }
