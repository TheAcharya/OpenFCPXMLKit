...
```bash
swift test --list-tests 2>/dev/null | grep -c '\.'                        # 951
swift test --list-tests 2>/dev/null | grep -c 'OpenFCPXMLKitTests\.'   # 948
...
- **FCPXMLReportPDFExportTests** — CoreGraphics PDF export: `%PDF` header, cover page and custom branding, table of contents for multi-section reports (TOC rows use sheet colour chips + tint washes), synthetic section content, wide-table multi-page pagination, full-workbook section parity (all nine sections in TOC/content), markers/media-summary/summary-only exports.
