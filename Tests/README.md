...
- **Test count:** **1166** tests listed in `swift test list` — **1159** in `OpenFCPXMLKitTests` + **7** in `ExcelReportTest` (all Swift Testing `@Test`; no XCTest remaining; ExcelReportTest cancels without a local fixture)
...
```bash
swift test list 2>/dev/null | grep -c '\.'                        # 1166
swift test list 2>/dev/null | grep -c 'OpenFCPXMLKitTests\.'   # 1159
...
**Keep counts in sync:** `swift test list` → **1166** total (**1159** OpenFCPXMLKitTests + **7** ExcelReportTest; all Swift Testing); **60** public samples.
