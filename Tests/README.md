...
- **Test count:** **1124** tests listed in `swift test list` — **1118** in `OpenFCPXMLKitTests` + **6** in `ExcelReportTest` (all Swift Testing `@Test`; no XCTest remaining; ExcelReportTest cancels without a local fixture)
...
```bash
swift test list 2>/dev/null | grep -c '\.'                        # 1124
swift test list 2>/dev/null | grep -c 'OpenFCPXMLKitTests\.'   # 1118
...
- **FCPXMLMarkersReportTests** — Markers report rows (type, position, clip name, role ▸ subrole), chapter-marker inclusion; default omission of out-of-bounds markers; `includeMarkersOutsideClipBoundaries` + **Hidden** column (✓/✗); sample `HiddenMarkers.fcpxml`.
...
**Keep counts in sync:** `swift test list` → **1124** total (**1118** OpenFCPXMLKitTests + **6** ExcelReportTest; all Swift Testing); **60** public samples.
