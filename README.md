...
**Tests:** **1244** listed in `swift test list` — **1230** in `OpenFCPXMLKitTests` + **10** optional `ExcelReportTest` + **4** optional `ShotExtractionTest` (all Swift Testing) — across **60** sample `.fcpxml` files. Private local investigation inbox: [`Tests/Submitted FCPXML/`](Tests/Submitted%20FCPXML/README.md) (gitignored; never commit private FCPXML).
...
let package = Package(
    ...
    dependencies: [
        .package(url: "https://github.com/TheAcharya/OpenFCPXMLKit", from: "3.3.11")
    ],
    targets: [
        .target(
            name: "MyTarget",
...
```shell
VERSION=3.3.11 # replace this with the git tag of the version you need
...
- **[Tests/README.md](Tests/README.md)** — Test suite layout (**1244** listed; all Swift Testing)
