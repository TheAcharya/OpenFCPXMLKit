...
- Reporting: Production's Best Friend–style Excel report builders live in Reporting/ and consume Extraction/Model; they map extracted facts to row/section models (role inventory, markers, keywords, titles, transitions, effects, speed-change effects, summary) and serialise to XLKit workbooks under Reporting/Excel/ via FinalCutPro.FCPXML.buildReport(options:) and ReportExcelExport. When FCPXML grows richer, extend Model/Parsing first, then Extraction, then Reporting presentation only. See ARCHITECTURE.md §2.7.
...
- Reporting: Production's Best Friend–style Excel workbook reports. Top-level: Report, ReportOptions, ReportBuilder, ReportBuildProgress. Subfolders: Builders (per-sheet: RoleInventory, Markers, Keywords, Titles, Transitions, Effects, SpeedChangeEffects, Summary), Sections and Rows (typed section/row models), Support (RoleInventoryClipCollector, RoleInventoryRowBuilder, RoleInventoryRoleSheetOrdering, RoleInventoryTimelineBounds, ReportFormatting, ReportRoleExclusion, ReportClipCategory, EffectsReportPolicy, SpeedChangeFormatting, SummaryRoleDurationAggregator), Excel (ReportExcelExport, ReportWorkbookExporter, ReportWorkbookColumnAutoFit via XLKit). Consumes Extraction; owns presentation only; see ARCHITECTURE.md §2.7 for layer boundaries.
