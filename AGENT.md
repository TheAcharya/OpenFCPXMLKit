...
Keep AGENT.md and .cursorrules in sync. Both must reflect: changelog styling (CHANGELOG.md: Keep a Changelog format, version links to release tags, ✨ New Features / 🔧 Improvements / 🐛 Bug Fixes). Hard must / must-not constraints live in **GUARDRAILS.md** (update Signs when a design lock or regression is learned — Signs: `swift-testing-only`, `chapter-markers-on-markers-sheet`, `markers-keywords-survive-host-occlusion`; keep ARCHITECTURE.md for structure and mermaid).
