...
//
//	Core FCPXML struct definition wrapping a OFKXMLDocument for high-level access.
//
...
extension FinalCutPro {
    ...
    /// > because it wraps `OFKXMLDocument` which is not `Sendable`. Use with caution in concurrent contexts.
    public struct FCPXML: Codable {
        /// The FCPXML document.
        public var xml: any OFKXMLDocument
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            // Convert OFKXMLDocument to XML string
        ...
        public init(from decoder: Decoder) throws {
            ...
            // Convert XML string to OFKXMLDocument
            guard let xmlData = xmlString.data(using: .utf8) else {
                throw FCPXMLCodableError.xmlStringConversionFailed
            }

            do {
                let xmlDocument = try OFKXMLDefaultFactory().makeDocument(data: xmlData, options: [])
