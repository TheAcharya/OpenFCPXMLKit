...
public final class FCPXMLUtility: Sendable {
	...
	/// Default instance used by extension APIs (e.g. `OFKXMLElement.fcpxDuration`, `OFKXMLDocument.fcpxAssetResources`) that cannot receive dependency injection. Concurrency-safe (immutable). For custom implementations use the modular API: inject dependencies into `FCPXMLService` / `FCPXMLUtility` and use the `+Modular` extensions (e.g. `element.setAttribute(name:value:using: documentManager)`) or service methods.
	...
	///
	/// - Parameters:
	///   - elements: An array of OFKXMLElement objects
	///   - types: An array of FCPXMLElementType enumeration values
	/// - Returns: A filtered array of OFKXMLElement objects
	public func filter(fcpxElements elements: [any OFKXMLElement], ofTypes types: [FCPXMLElementType]) -> [any OFKXMLElement] {
	...
	///   - types: An array of FCPXMLElementType enumeration values
	/// - Returns: A filtered array of OFKXMLElement objects
	public func filterElements(_ elements: [any OFKXMLElement], ofTypes types: [FCPXMLElementType]) -> [any OFKXMLElement] {
	...
	/// - Parameter spine: An FCPXML `spine` OFKXMLElement.
	...
	public func convertToVersion(_ document: any OFKXMLDocument, targetVersion: FCPXMLVersion) throws -> any OFKXMLDocument {
		try versionConverter.convert(document, to: targetVersion)
	}

	/// Saves the document as a single .fcpxml file.
	public func saveAsFCPXML(_ document: any OFKXMLDocument, to url: URL) throws {
	...
	public func saveAsBundle(_ document: any OFKXMLDocument, to outputDirectory: URL, bundleName: String) throws -> URL {
	...
	public func convertToVersion(_ document: any OFKXMLDocument, targetVersion: FCPXMLVersion) async throws -> any OFKXMLDocument {
		try await versionConverter.convert(document, to: targetVersion)
	}

	/// Asynchronously saves the document as a .fcpxml file.
	public func saveAsFCPXML(_ document: any OFKXMLDocument, to url: URL) async throws {
	...
	public func saveAsBundle(_ document: any OFKXMLDocument, to outputDirectory: URL, bundleName: String) async throws -> URL {
		let exporter = FCPXMLBundleExporter(version: .v1_10)
		return try exporter.saveDocumentAsBundle(document, to: outputDirectory, bundleName: bundleName)
	}

	/// Extracts media references (asset media-rep src, locator url) from the document.
	public func extractMediaReferences(from document: any OFKXMLDocument, baseURL: URL? = nil) -> MediaExtractionResult {
	...
	public func extractMediaReferences(from document: any OFKXMLDocument, baseURL: URL? = nil) async -> MediaExtractionResult {
	...
	public func validateDocument(_ document: any OFKXMLDocument) -> Bool {
	...
	public func validateDocumentAgainstDTD(_ document: any OFKXMLDocument, version: FCPXMLVersion) -> ValidationResult {
	...
	public func validateDocumentAgainstDeclaredVersion(_ document: any OFKXMLDocument) -> ValidationResult {
	...
	public func performValidation(_ document: any OFKXMLDocument) -> DocumentValidationReport {
		_performValidation(document)
	}

	private func _performValidation(_ document: any OFKXMLDocument) -> DocumentValidationReport {
	...
	private func _validateDocumentAgainstDeclaredVersion(_ document: any OFKXMLDocument) -> ValidationResult {
	...
	public func validateDocument(_ document: any OFKXMLDocument) async -> Bool {
	...
	/// and works with non-Sendable `OFKXMLDocument` types, wrapping in `Task` would not provide
	...
	///   - version: The FCPXML version whose DTD to use (e.g. `.v1_10`, `.v1_14`).
	/// - Returns: `.success` if the document conforms to that version's DTD; otherwise a result with `dtd_validation` error(s).
	public func validateDocumentAgainstDTD(_ document: any OFKXMLDocument, version: FCPXMLVersion) async -> ValidationResult {
	...
	/// - Parameter document: The FCPXML document to validate.
	/// - Returns: `.success` if the document conforms to its declared version's DTD; otherwise a result with errors (e.g. unknown version or DTD validation failure).
	public func validateDocumentAgainstDeclaredVersion(_ document: any OFKXMLDocument) async -> ValidationResult {
		_validateDocumentAgainstDeclaredVersion(document)
	}
	...
	/// - Parameter document: The FCPXML document to validate.
	/// - Returns: A report combining semantic and DTD results; ``DocumentValidationReport/isValid`` is `true` only when both pass.
	public func performValidation(_ document: any OFKXMLDocument) async -> DocumentValidationReport {
		_performValidation(document)
	}
	...
	public func projectTimecode(fromCounterValue counterValue: CMTime, inProject project: any OFKXMLElement) -> CMTime? {
	...
	public func projectCounterTime(fromTimecodeValue timecodeValue: CMTime, inProject project: any OFKXMLElement) -> CMTime? {
		guard let projectSequence = project.fcpxProjectSequence else {
			return nil
		}
    ...
    /// - Returns: An optional CMTime value of the timecode value.
    public func sequenceTimecode(fromCounterValue counterValue: CMTime, inSequence sequence: any OFKXMLElement) -> CMTime? {
    ...
    ///   - sequence: The sequence to convert against, as an NSXMLElement.
    /// - Returns: An optional CMTime value of the counter time.
    public func sequenceCounterTime(fromTimecodeValue timecodeValue: CMTime, inSequence sequence: any OFKXMLElement) -> CMTime? {
	...
	public func parentTime(fromLocalTime localTimeValue: CMTime, forClip clip: any OFKXMLElement) -> CMTime? {
	...
	///   - forClip: The clip to convert against.
	/// - Returns: A CMTime value of the resulting parent time value.
	public func localTime(fromParentTime parentTimeValue: CMTime, forClip clip: any OFKXMLElement) -> CMTime? {
	...
	public func projectTime(forClip clip: any OFKXMLElement, inProject project: any OFKXMLElement) -> CMTime? {
		...
		guard let clipParentElement = clip.parent else {
			debugLog("clipParentElement is nil or not OFKXMLElement")
			return nil
		}
		if clipParentElement.name == "spine" && clipParentElement.fcpxOffset != nil { // If the clip is in a secondary storyline
			...
			guard let spineParent = clipParentElement.parent else {
				debugLog("spineParent is nil or not OFKXMLElement")
				return nil
			}
	...
	/// Returns the clip's parent's equivalent offset timings for the specified in and out times. This is useful for walking up a OFKXMLElement hierarchy in order to get the time values of the clip on the project timeline.
	///
	/// - Parameters:
	...
	/// - Returns: A tuple of the converted in time, the converted out time, and the parent OFKXMLElement of the specified clip.
	public func parentClipTime(forInTime inTime: CMTime, outTime: CMTime, forClip clip: any OFKXMLElement) -> (in: CMTime, out: CMTime, parent: any OFKXMLElement)? {
	...
	/// - Returns: A OFKXMLDocument or nil if there was a file read or conversion error.
	public func convertLineBreaksInAttributes(inXMLDocumentURL url: URL) async -> (any OFKXMLDocument)? {
		...
		do {
			...
			let newXMLDocument = try OFKXMLDefaultFactory().makeDocument(xmlString: newDocument, options: [.preserveWhitespace, .prettyPrint, .compactEmptyElements])
