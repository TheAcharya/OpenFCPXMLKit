...
extension FinalCutPro.FCPXML.Effect {
    ...
    /// True when the effect UID indicates an Apple-supplied Motion template or FxPlug effect.
    public var isAppleSupplied: Bool {
        Self.isAppleSupplied(uid: uid)
    }
    
    /// Returns true when a UID indicates an Apple-supplied Motion template or FxPlug effect.
    public static func isAppleSupplied(uid: String) -> Bool {
        guard !uid.isEmpty else { return false }
        
        if uid.hasPrefix("FxPlug:") { return true }
        if uid.hasPrefix(".../") { return true }
        if uid.contains(".localized/"), !uid.hasPrefix("~/") { return true }
