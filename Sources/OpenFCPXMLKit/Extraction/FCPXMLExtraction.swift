//
//  FCPXMLExtraction.swift
//  OpenFCPXMLKit • https://github.com/TheAcharya/OpenFCPXMLKit
//  © 2026 • Licensed under MIT License
//

//
//	Public extraction API extension on OFKXMLElement.
//
...
// MARK: - OFKXMLElement Public Methods
...
extension OFKXMLElement {
    ...
    func _fcpExtract<Ancestors: Sequence<any OFKXMLElement>>(
        types elementTypes: Set<FinalCutPro.FCPXML.ElementType>,
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors,
    ...
    ///   - overrideDirectChildren: Uses the direct children rule supplied instead of the default
    ///     rule for the element type.
    private func _fcpExtract<Ancestors: Sequence<any OFKXMLElement>>(
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors,
        resources: (any OFKXMLElement)?,
        overrideDirectChildren: FinalCutPro.FCPXML.ExtractableChildren? = nil
    ...
    private func _fcpExtractPeers<Ancestors: Sequence<any OFKXMLElement>>(
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors,
        resources: (any OFKXMLElement)?
    ) async -> [FinalCutPro.FCPXML.ExtractedElement] {
        ...
        for element in elements {
            ...
            output.append(contentsOf: extracted)
        }
        return output
    }
    ...
    private func _fcpExtractDirectChildren<Ancestors: Sequence<any OFKXMLElement>>(
        childrenRule: FinalCutPro.FCPXML.ExtractableChildren.DirectChildren,
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors,
        resources: (any OFKXMLElement)?
    ) async -> [FinalCutPro.FCPXML.ExtractedElement] {
        let childrenSource: any Sequence<any OFKXMLElement>
        switch childrenRule {
        case .all:
    ...
    private func _fcpExtractDescendants<Ancestors: Sequence<any OFKXMLElement>>(
        descendants: [FinalCutPro.FCPXML.ExtractableChildren.Descendant],
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors,
        resources: (any OFKXMLElement)?
    ) async -> [FinalCutPro.FCPXML.ExtractedElement] {
        var descendantAccum: [any OFKXMLElement] = []
        var output: [FinalCutPro.FCPXML.ExtractedElement] = []
        for next in descendants {
            let extracted = await next.element._fcpExtract(
                ...
                resources: resources,
                overrideDirectChildren: next.children
            )
            output.append(contentsOf: extracted)
            descendantAccum.insert(next.element, at: 0)
        }
        return output
    }
    ...
    ///
    /// Ancestors are ordered nearest to furthest ancestor.
    static func _fcpShouldKeepForExtraction<Ancestors: Sequence<any OFKXMLElement>>(
    ...
    ///
    /// Ancestors are ordered nearest to furthest ancestor.
    static func _fcpShouldKeepForTraversal<Ancestors: Sequence<any OFKXMLElement>>(
        extractedElement: FinalCutPro.FCPXML.ExtractedElement,
        scope: FinalCutPro.FCPXML.ExtractionScope,
        ancestors: Ancestors
    ) -> Bool {
    ...
    ///
    /// Ancestors are ordered nearest to furthest ancestor.
    static func _fcpContainerDepth<Ancestors: Sequence<any OFKXMLElement>>(
        ...
        for ancestor in ancestors {
            ...
            if elementType == .assetClip || elementType == .refClip {
                if isTraversingContainerClip {
                    ...
                    // Note: may need to extend to other clip types in the future.
                    continue
                }
...
extension OFKXMLElement {
    ...
    ///
    /// Ancestors are ordered nearest to furthest ancestor.
    func _fcpEffectiveLane<Ancestors: Sequence<any OFKXMLElement>>(
        ancestors: Ancestors
    ) -> Int? {
