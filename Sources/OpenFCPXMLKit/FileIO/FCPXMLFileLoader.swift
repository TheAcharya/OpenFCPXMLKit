...
public struct FCPXMLFileLoader: Sendable {
    ...
    /// For a `.fcpxmld` bundle, reads `Info.fcpxml` inside the bundle.
    public func loadDocument(from url: URL) throws -> any OFKXMLDocument {
    ...
    /// Loads an FCPXML document using OpenFCPXMLKit's FCPXML parsing options (preserve whitespace, pretty print).
    public func loadFCPXMLDocument(from url: URL) throws -> any OFKXMLDocument {
        let data = try loadData(from: url)
        do {
    ...
    public func load(from url: URL) async throws -> any OFKXMLDocument {
