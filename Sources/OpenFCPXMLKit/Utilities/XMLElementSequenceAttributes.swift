...
extension Sequence where Element == any OFKXMLElement {
    ...
    func first(withAttribute name: String) -> (element: any OFKXMLElement, attributeValue: String)? {
        for element in self {
    ...
    func first(whereAttribute name: String, hasValue value: String) -> (any OFKXMLElement)? {
        for element in self {
