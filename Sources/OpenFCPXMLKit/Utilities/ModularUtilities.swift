...
//
//	Service creation, validation, and batch processing helpers.
//
...
public struct ModularUtilities: Sendable {
    ...
    /// Creates a complete FCPXML processing service with all dependencies
    /// - Returns: Configured FCPXMLService
    public static func createService() -> FCPXMLService {
    ...
    /// Creates a custom service with specific implementations
    /// - Parameters:
    ...
    public static func validateDocument(_ document: any OFKXMLDocument) -> (isValid: Bool, errors: [String]) {
    ...
    public static func validateDocument(_ document: any OFKXMLDocument, using parser: FCPXMLParsing) -> (isValid: Bool, errors: [String]) {
    ...
    ) -> Result<any OFKXMLDocument, FCPXMLError> {
        do {
    ...
    /// - Parameter document: Document to validate.
    /// - Returns: Validation result with error messages.
    public static func validateDocument(_ document: any OFKXMLDocument) async -> (isValid: Bool, errors: [String]) {
        let result = sharedValidator.validate(document)
        let errorMessages = result.errors.map(\.message)
        return (result.isValid, errorMessages)
    }
    ...
    @available(*, deprecated, message: "Use validateDocument(_:) without the parser parameter.")
    public static func validateDocument(_ document: any OFKXMLDocument, using parser: FCPXMLParsing) async -> (isValid: Bool, errors: [String]) {
        let result = sharedValidator.validate(document)
        let errorMessages = result.errors.map(\.message)
        return (result.isValid, errorMessages)
    }
    ...
    public static func processFCPXML(
        from url: URL,
        using service: FCPXMLService
    ) async -> Result<any OFKXMLDocument, FCPXMLError> {
        do {
    ...
    public static func processMultipleFCPXML(
        ...
        var results: [Result<any OFKXMLDocument, FCPXMLError>] = []
        for url in urls {
    ...
    ///
    /// - Parameters:
    ///   - elements: Array of OFKXMLElements with time attributes
    ...
    public static func convertTimecodes(
        for elements: [any OFKXMLElement],
