...
//
//	OFKXMLElement extension for walking ancestor elements.
//
...
private func _walkAncestorElements<T>(
    startingWith element: (any OFKXMLElement)?,
    returning: T.Type,
    block: (_ element: any OFKXMLElement) -> WalkAncestorsIntermediateResult<T>
...
// MARK: - OFKXMLElement Ancestor Walking

extension OFKXMLElement {
    /// Walk ancestors of the element.
    func walkAncestorElements(
        includingSelf: Bool,
        _ block: (_ element: any OFKXMLElement) -> Bool
    ) {
        let blockWithResult: (_ element: any OFKXMLElement) -> WalkAncestorsIntermediateResult<Void> = { element in
            if block(element) {
                return .continue
            } else {
    ...
    func walkAncestorElements<T>(
        includingSelf: Bool,
        returning: T.Type,
        _ block: (_ element: any OFKXMLElement) -> WalkAncestorsIntermediateResult<T>
    ) -> WalkAncestorsResult<T> {
...
extension OFKXMLElement {
    ...
    func ancestorElements<S: Sequence<any OFKXMLElement>>(
        overrideWith replacement: S?,
        includingSelf: Bool
    ) -> AnySequence<any OFKXMLElement> {
