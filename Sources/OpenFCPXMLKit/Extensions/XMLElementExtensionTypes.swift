//
//  XMLElementExtensionTypes.swift
//  OpenFCPXMLKit • https://github.com/TheAcharya/OpenFCPXMLKit
//  © 2026 • Licensed under MIT License
//

//
//	Enums and types formerly nested in the OFKXMLElement extension.
//	Swift does not allow types to be nested in protocol extensions,
//	so they are declared at file scope and typealiased back.
//

import Foundation

// MARK: - FCPXMLElementError

/// Errors that can occur during FCPXML element operations.
enum FCPXMLElementError: Error, CustomStringConvertible, Sendable {
    case notAnEvent(elementName: String)
    case notAnAnnotatableItem(elementName: String)
    case notAnAnnotation(elementName: String)

    var description: String {
        switch self {
        case .notAnEvent(let elementName):
            return "The \"\(elementName)\" element is not an event."
        case .notAnAnnotatableItem(let elementName):
            return "The \"\(elementName)\" element cannot be annotated."
        case .notAnAnnotation(let elementName):
            return "The \"\(elementName)\" element is not an annotation."
        }
    }
}

// MARK: - OFKXMLElementTimecodeFormat

/// Legacy timecode format enum used by the OFKXMLElement extension.
/// Equivalent to `FinalCutPro.FCPXML.TimecodeFormat` but kept for
/// backward compatibility with the legacy XMLElement API surface.
public enum OFKXMLElementTimecodeFormat: String, Sendable {
    case dropFrame = "DF"
    case nonDropFrame = "NDF"
}

// MARK: - OFKXMLElementAudioLayout

/// Legacy audio layout enum used by the OFKXMLElement extension.
public enum OFKXMLElementAudioLayout: String, Sendable {
    case mono = "mono"
    case stereo = "stereo"
    case surround = "surround"
}

// MARK: - OFKXMLElementAudioRate

/// Legacy audio rate enum used by the OFKXMLElement extension.
/// Uses sequence-style raw values ("32k", "48k", etc.).
public enum OFKXMLElementAudioRate: String, Sendable {
    case rate32kHz = "32k"
    case rate44_1kHz = "44.1k"
    case rate48kHz = "48k"
    case rate88_2kHz = "88.2k"
    case rate96kHz = "96k"
    case rate176_4kHz = "176.4k"
    case rate192kHz = "192k"
}

// MARK: - OFKXMLElementRenderColorSpace

/// Legacy render color space for existing FCPXML project/sequence attributes.
///
/// For new export workflows, prefer ``ColorSpace`` which includes HDR variants
/// and provides `fcpxmlValue` for attribute serialisation.
public enum OFKXMLElementRenderColorSpace: String, Sendable {
    case rec601NTSC = "Rec. 601 (NTSC)"
    case rec601PAL = "Rec. 601 (PAL)"
    case rec709 = "Rec. 709"
    case rec2020 = "Rec. 2020"
}

// MARK: - OFKXMLElementMulticamSourceEnable

public enum OFKXMLElementMulticamSourceEnable: String, Sendable {
    case audio = "audio"
    case video = "video"
    case all = "all"
    case none = "none"
}

// MARK: - OFKXMLElementCaptionFormat

/// The caption format included in caption role attributes.
public enum OFKXMLElementCaptionFormat: String, Sendable {
    case itt = "ITT"
    case cea608 = "CEA608"
}

// MARK: - OFKXMLElementCaptionLanguage

/// RFC 5646 language tags for use in caption role attributes.
/// The languages included in this enum are those supported by FCPX.
public enum OFKXMLElementCaptionLanguage: String, Sendable {
    case afrikaans = "af"
    case arabic = "ar"
    case bangla = "bn"
    case bulgarian = "bg"
    case catalan = "ca"
    case chineseCantonese = "yue-Hant"
    case chineseSimplified = "cmn-Hans"
    case chineseTraditional = "cmn-Hant"
    case croatian = "hr"
    case czech = "cs"
    case danish = "da"
    case dutch = "nl"
    case english = "en"
    case englishAustralia = "en-AU"
    case englishCanada = "en-CA"
    case englishUnitedKingdom = "en-GB"
    case englishUnitedStates = "en-US"
    case estonian = "et"
    case finnish = "fi"
    case frenchBelgium = "fr-BE"
    case frenchCanada = "fr-CA"
    case frenchFrance = "fr-FR"
    case frenchSwitzerland = "fr-CH"
    case german = "de"
    case germanAustria = "de-AT"
    case germanGermany = "de-DE"
    case germanSwitzerland = "de-CH"
    case greek = "el"
    case greekCyprus = "el-CY"
    case hebrew = "he"
    case hindi = "hi"
    case hungarian = "hu"
    case icelandic = "is"
    case indonesian = "id"
    case italian = "it"
    case japanese = "ja"
    case kannada = "kn"
    case kazakh = "kk"
    case korean = "ko"
    case lao = "lo"
    case latvian = "lv"
    case lithuanian = "lt"
    case luxembourgish = "lb"
    case malay = "ms"
    case malayalam = "ml"
    case maltese = "mt"
    case marathi = "mr"
    case norwegian = "no"
    case polish = "pl"
    case portugueseBrazil = "pt-BR"
    case portuguesePortugal = "pt-PT"
    case punjabi = "pa"
    case romanian = "ro"
    case russian = "ru"
    case slovak = "sk"
    case slovenian = "sl"
    case spanishLatinAmerica = "es-419"
    case spanishMexico = "es-MX"
    case spanishSpain = "es-ES"
    case swedish = "sv"
    case tagalog = "tl"
    case tamil = "ta"
    case telugu = "te"
    case thai = "th"
    case turkish = "tr"
    case ukrainian = "uk"
    case urdu = "ur"
    case vietnamese = "vi"
    case zulu = "zu"
}

// MARK: - OFKXMLElementCEA608CaptionDisplayStyle

/// Caption display style for CEA-608 captions
public enum OFKXMLElementCEA608CaptionDisplayStyle: String, Sendable {
    case popOn = "pop-on"
    case paintOn = "paint-on"
    case rollUp = "roll-up"
}

// MARK: - OFKXMLElementITTCaptionPlacement

/// Caption placement for ITT captions.
public enum OFKXMLElementITTCaptionPlacement: String, Sendable {
    case top = "top"
    case bottom = "bottom"
    case left = "left"
    case right = "right"
}

// MARK: - OFKXMLElementCEA608CaptionAlignment

/// Caption alignment for CEA-608 captions.
public enum OFKXMLElementCEA608CaptionAlignment: String, Sendable {
    case left = "left"
    case center = "center"
    case right = "right"
}

// MARK: - OFKXMLElementCEA608Color

/// Color values for CEA-608 captions. The raw value is the color expressed as
/// "red green blue alpha" which is the way it is represented in FCPXML text style elements.
public enum OFKXMLElementCEA608Color: String, Sendable {
    case red = "1 0 0 1"
    case yellow = "1 1 0 1"
    case green = "0 1 0 1"
    case cyan = "0 1 1 1"
    case blue = "0 0 1 1"
    case magenta = "1 0 1 1"
    case white = "1 1 1 1"
    case black = "0 0 0 1"
}

// MARK: - OFKXMLElementStoryElementLocation

/// The location of a story element within its sequence or timeline.
///
/// - primaryStoryline: The story element exists on the primary storyline.
/// - attachedClip: The story element is attached to another clip that is on the primary storyline.
/// - secondaryStoryline: The story element is embedded in a secondary storyline.
public enum OFKXMLElementStoryElementLocation: Sendable {
    case primaryStoryline
    case attachedClip
    case secondaryStoryline
}

// MARK: - OFKXMLElement Typealiases

extension OFKXMLElement {
    // These typealiases allow existing code to use e.g. `TimecodeFormat`
    // unqualified within OFKXMLElement extension methods, preserving the
    // original API shape.

    typealias FCPXMLElementError = OpenFCPXMLKit.FCPXMLElementError
    public typealias TextAlignment = FinalCutPro.FCPXML.TextAlignment
    public typealias TimecodeFormat = OFKXMLElementTimecodeFormat
    public typealias AudioLayout = OFKXMLElementAudioLayout
    public typealias AudioRate = OFKXMLElementAudioRate
    public typealias RenderColorSpace = OFKXMLElementRenderColorSpace
    public typealias MulticamSourceEnable = OFKXMLElementMulticamSourceEnable
    public typealias CaptionFormat = OFKXMLElementCaptionFormat
    public typealias CaptionLanguage = OFKXMLElementCaptionLanguage
    public typealias CEA608CaptionDisplayStyle = OFKXMLElementCEA608CaptionDisplayStyle
    public typealias ITTCaptionPlacement = OFKXMLElementITTCaptionPlacement
    public typealias CEA608CaptionAlignment = OFKXMLElementCEA608CaptionAlignment
    public typealias CEA608Color = OFKXMLElementCEA608Color
    public typealias StoryElementLocation = OFKXMLElementStoryElementLocation
}
