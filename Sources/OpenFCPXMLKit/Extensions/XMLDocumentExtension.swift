...
//
//	OFKXMLDocument extensions for FCPXML document-level operations.
//
...
extension FoundationXMLDocument {
	...
	///
	/// - Parameters:
	///   - resources: Resources an array of OFKXMLElement objects
	///   - events: Events as an array of OFKXMLElement objects
	///   - fcpxmlVersion: The FCPXML version of the document to use.
	public convenience init(resources: [any OFKXMLElement], events: [any OFKXMLElement], fcpxmlVersion: String) {
		self.init()
	...
	///
	/// - Parameters:
	///   - resources: Resources as an array of OFKXMLElement objects.
	///   - events: Events as an array of OFKXMLElement objects.
	///   - fcpxmlVersion: The FCPXML version (e.g. `.default` or `.v1_14`).
	public convenience init(resources: [any OFKXMLElement], events: [any OFKXMLElement], fcpxmlVersion: FCPXMLVersion) {
	...
	public var fcpxmlElement: (any OFKXMLElement)? {
		...
		for child in children {
			if child.name == "fcpxml" {
				if let element = child as? (any OFKXMLElement) {
					return element
				}
			}
		}
	...
	public var fcpxResourceElement: (any OFKXMLElement)? {
	...
	public var fcpxResources: [any OFKXMLElement] {
	...
	public var fcpxFormatResources: [any OFKXMLElement] {
	...
	public var fcpxAssetResources: [any OFKXMLElement] {
		let utility = FCPXMLUtility.defaultForExtensions
	...
	public var fcpxMulticamResources: [any OFKXMLElement] {
		let utility = FCPXMLUtility.defaultForExtensions
	...
	public var fcpxCompoundResources: [any OFKXMLElement] {
		let utility = FCPXMLUtility.defaultForExtensions
	...
	public var fcpxEffectResources: [any OFKXMLElement] {
		let utility = FCPXMLUtility.defaultForExtensions
	...
	public var fcpxAllProjects: [any OFKXMLElement] {
		var projects: [any OFKXMLElement] = []
		let utility = FCPXMLUtility.defaultForExtensions
	...
	public var fcpxAllClips: [any OFKXMLElement] {
	...
	/// All items from all events as a OFKXMLElement array.
	public var fcpxAllEventItems: [any OFKXMLElement] {
		var allItems: [any OFKXMLElement] = []
	...
	public func resource(matchingID id: String) -> (any OFKXMLElement)? {
	...
	/// - Returns: An array of OFKXMLElement objects that are matching asset resources.
	public func assetResources(matchingURL url: URL, usingFilenameOnly: Bool, omittingExtension: Bool, caseSensitive: Bool) -> [any OFKXMLElement] {
		...
		var matchingResources: [any OFKXMLElement] = []
	...
	///
	/// - Parameter resourceElements: An array of OFKXMLElement objects to add as resources.
	public func add(resourceElements: [any OFKXMLElement]) {
	...
	///
	/// - Parameter events: An array of OFKXMLElement objects to add as events.
	public func add(events: [any OFKXMLElement]) {
		guard let libraryElement = self.fcpxLibraryElement else {
			return
		}
	...
	/// Returns the OpenFCPXMLKit resource bundle when it lives next to the running executable (e.g. a copied CLI) or next to the test bundle (e.g. swift test).
	private static func _openFCPXMLKitBundleNextToExecutable() -> Bundle? {
		...
		let bundleNames = ["OpenFCPXMLKit_OpenFCPXMLKitCLI.bundle", "OpenFCPXMLKit_OpenFCPXMLKit.bundle"]
	...
	/// Skip only when the running executable is the OpenFCPXMLKit-CLI CLI (by name) and the resource bundle is not next to it.
	...
	private static func _mustSkipBundleModule() -> Bool {
		...
		guard execName == "OpenFCPXMLKit-CLI" else { return false }
		let execDir: URL
		if resolvedPath.hasPrefix("/") {
			execDir = URL(fileURLWithPath: resolvedPath).deletingLastPathComponent()
		} else {
		...
		let cliBundle = execDir.appendingPathComponent("OpenFCPXMLKit_OpenFCPXMLKitCLI.bundle")
		let libBundle = execDir.appendingPathComponent("OpenFCPXMLKit_OpenFCPXMLKit.bundle")
	...
	private func setDTDToBundleResource(named name: String) throws {
		...
		var dtdURL: URL? = nil
		let bundleNextToExecutable = Self._openFCPXMLKitBundleNextToExecutable()
	...
	private func xmlElementArrayToStringArray(usingXMLArray xmlArray: [any OFKXMLElement]) -> [String] {
		xmlArray.map { $0.xmlString }
	}

	private func stringArrayToXMLElementArray(usingStringArray stringArray: [String]) -> [any OFKXMLElement] {
	...
	///
	/// - DTDResourceNotFound: The DTD resource in the OpenFCPXMLKit framework was not found.
	/// - dtdResourceUnreadable: The DTD resource in the OpenFCPXMLKit framework was not readable.
...
#if canImport(Logging)
  	private static let logger = Logger(label: "OpenFCPXMLKit.FoundationXMLDocument")
