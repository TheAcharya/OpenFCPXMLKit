...
//  OpenFCPXMLKit
//
//  Foundation backend adapter for OFKXMLDocument.
...
/// Maps `OFKXMLDocumentOptions` to Foundation's `XMLNode.Options`.
private func foundationOptions(from options: OFKXMLDocumentOptions) -> XMLNode.Options {
...
/// A Foundation-backed XML document that conforms to `OFKXMLDocument`.
///
...
public final class FoundationXMLDocument: OFKXMLDocument {
    ...
    // MARK: - OFKXMLDocument Parsing Initializers
    ...
    public convenience init(data: Data, options: OFKXMLDocumentOptions) throws {
        do {
            let doc = try XMLDocument(data: data, options: foundationOptions(from: options))
            self.init(doc)
        } catch {
            throw OFKXMLError.parsingFailure(message: error.localizedDescription)
        }
    }
    ...
    ///   - options: Options controlling parsing behavior.
    /// - Throws: `OFKXMLError.parsingFailure` if parsing fails.
    public convenience init(contentsOf url: URL, options: OFKXMLDocumentOptions) throws {
        do {
    ...
    ///   - options: Options controlling parsing behavior.
    /// - Throws: `OFKXMLError.parsingFailure` if parsing fails.
    public convenience init(xmlString: String, options: OFKXMLDocumentOptions) throws {
        do {
    ...
    public var parent: (any OFKXMLElement)? {
        // Documents have no parent element.
        nil
    }

    public var children: [any OFKXMLNode]? {
        underlyingDocument.children?.map { child in
            if let element = child as? XMLElement {
                return FoundationXMLElement(element) as any OFKXMLNode
            }
            return FoundationXMLNode(child) as any OFKXMLNode
        }
    }

    // MARK: - OFKXMLDocument: Document Structure

    public func rootElement() -> (any OFKXMLElement)? {
        guard let root = underlyingDocument.rootElement() else { return nil }
        return FoundationXMLElement(root)
    }

    public func setRootElement(_ root: any OFKXMLElement) {
    ...
    public var documentContentKind: OFKXMLDocumentContentKind {
        get {
    ...
    // MARK: - OFKXMLDocument: Serialization

    public func xmlData(options: OFKXMLDocumentOptions) -> Data {
    ...
    // MARK: - OFKXMLDocument: DTD

    public var dtd: (any OFKXMLDTDProtocol)? {
        get {
