//
//  OFKXMLDocument.swift
//  OpenFCPXMLKit
//
//  Platform-agnostic XML document protocol.
//  Mirrors the Foundation XMLDocument API surface used by OpenFCPXMLKit:
...
public struct OFKXMLDocumentOptions: OptionSet, Sendable, Hashable {
    ...
    public static let preserveWhitespace = OFKXMLDocumentOptions(rawValue: 1 << 0)

    /// Pretty-print the output with indentation (maps to `XMLNode.Options.nodePrettyPrint`).
    public static let prettyPrint = OFKXMLDocumentOptions(rawValue: 1 << 1)

    /// Use compact empty-element tags (`<br/>` instead of `<br></br>`)
    /// (maps to `XMLNode.Options.nodeCompactEmptyElement`).
    public static let compactEmptyElements = OFKXMLDocumentOptions(rawValue: 1 << 2)

    /// The default options used throughout OpenFCPXMLKit for FCPXML handling.
    /// Equivalent to `[.nodePreserveWhitespace, .nodePrettyPrint, .nodeCompactEmptyElement]`.
    public static let fcpxmlDefaults: OFKXMLDocumentOptions = [
        .preserveWhitespace,
        .prettyPrint,
...
public protocol OFKXMLDocument: OFKXMLNode {
    ...
    init(data: Data, options: OFKXMLDocumentOptions) throws
    ...
    init(contentsOf url: URL, options: OFKXMLDocumentOptions) throws
    ...
    init(xmlString: String, options: OFKXMLDocumentOptions) throws
    ...
    func rootElement() -> (any OFKXMLElement)?
    ...
    func setRootElement(_ root: any OFKXMLElement)
    ...
    var documentContentKind: OFKXMLDocumentContentKind { get set }
    ...
    func xmlData(options: OFKXMLDocumentOptions) -> Data
    ...
    var dtd: (any OFKXMLDTDProtocol)? { get set }

    /// Validates the document against its DTD.
    ///
    /// - Throws: `OFKXMLError` if validation fails. On platforms without DTD support,
