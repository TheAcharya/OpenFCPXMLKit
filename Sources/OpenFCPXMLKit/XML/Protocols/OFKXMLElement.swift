...
//  Mirrors the Foundation XMLElement API surface used by OpenFCPXMLKit,
...
///
/// Conforms to `OFKXMLNode` and adds attribute access, child manipulation,
...
public protocol OFKXMLElement: OFKXMLNode {
    ...
    func elements(forName name: String) -> [any OFKXMLElement]

    /// Appends a child node to this element.
    ///
    /// Mirrors `XMLElement.addChild(_:)`.
    func addChild(_ child: any OFKXMLNode)
    ...
    func insertChild(_ child: any OFKXMLNode, at index: Int)

    // MARK: - Serialization

    /// The XML string representation, formatted with indentation.
    ///
    /// Inherited from `OFKXMLNode.xmlString`.
    // var xmlString: String { get }  // already declared in OFKXMLNode
...
extension OFKXMLElement {
    ...
    public func addChildren(_ children: [any OFKXMLNode]) {
    ...
    public func removeChildren(
        where shouldBeRemoved: (_ child: any OFKXMLElement) throws -> Bool
    ) rethrows {
    ...
    public func ancestorElements(includingSelf: Bool) -> [any OFKXMLElement] {
        var result: [any OFKXMLElement] = []
        if includingSelf {
            result.append(self)
        }
        var current: (any OFKXMLElement)? = self.parent
