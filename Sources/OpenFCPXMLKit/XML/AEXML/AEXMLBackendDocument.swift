...
//  Wraps `AEXMLDocument` and conforms to `OFKXMLDocument`.
//
...
//  - `validate()` → throws `OFKXMLError.dtdValidationUnavailable` (AEXML has
//    no DTD support).
//
...
public final class AEXMLBackendDocument: OFKXMLDocument {
    ...
    public var documentContentKind: OFKXMLDocumentContentKind = .xml
    ...
    public convenience init(data: Data, options: OFKXMLDocumentOptions) throws {
        ...
        } catch let error as OFKXMLError {
            throw error
        } catch {
            throw OFKXMLError.parsingFailure(
    ...
    public var parent: (any OFKXMLElement)? { nil }
    ...
    // MARK: - OFKXMLDocument: DTD (platform-conditional)
    ...
    /// Maps `OFKXMLDocumentOptions` to `AEXMLOptions`.
    ///
    ...
    private static func aeOptions(from options: OFKXMLDocumentOptions) -> AEXMLOptions {
