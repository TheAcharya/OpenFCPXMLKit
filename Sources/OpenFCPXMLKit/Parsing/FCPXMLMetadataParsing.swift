...
extension OFKXMLElement {
    ...
    /// metadata `key`.
    func _fcpMetadataChild(forKey key: FinalCutPro.FCPXML.Metadata.Key) -> (any OFKXMLElement)? {
    ...
    /// metadata `key`.
    @_disfavoredOverload
    func _fcpMetadataChild(forKey key: String) -> (any OFKXMLElement)? {
        childElements
    ...
    func _fcpUpdateMetadataChild(
        ...
        if let newValue {
                ...
                .first(whereAttribute: "key", hasValue: key)
            {
                ...
                let newMDElement = OFKXMLDefaultFactory().makeElement(name: "md")
...
extension OFKXMLElement {
    ...
    func _fcpUpdateMetadataChild(
        ...
        if let newArray, !newArray.isEmpty {
            ...
            var mdElement: (any OFKXMLElement)? = childElements
                .filter(whereElementNamed: "md")
                .first(whereAttribute: "key", hasValue: key)
...
extension OFKXMLElement {
    ...
    func _setFirstChildStringArray(_ newArray: [String]?) {
        ...
        let arrayElement = existingArrayElement ?? {
            let newArrayElement = OFKXMLDefaultFactory().makeElement(name: "array")
        ...
        for newArrayString in newArray {
            let newArrayElement = OFKXMLDefaultFactory().makeElement(name: "")
