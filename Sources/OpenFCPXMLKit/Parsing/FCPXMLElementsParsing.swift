...
extension OFKXMLElement {
    ...
    public var fcpStructureElements: [any OFKXMLElement] {
        childElements
...
extension OFKXMLElement {
    ...
    public var fcpStoryElements: [any OFKXMLElement] {
        childElements
...
extension OFKXMLElement {
    ...
    public var fcpTimelineElements: [any OFKXMLElement] {
        childElements
...
extension OFKXMLElement {
    ...
    func _updateChildElements<S: Sequence>(
        ...
        with newChildren: S
    ) where S.Element == any OFKXMLNode {
    ...
    func _updateChildElements<S: Sequence>(
        ofType elementType: FinalCutPro.FCPXML.ElementType,
        with newChildren: S
    ) where S.Element == any OFKXMLElement {
        // remove existing children of the element type first
        removeChildren { child in
            child.fcpElementType == elementType
        }
        ...
        // add new children
        addChildren(newChildren.map { $0 as any OFKXMLNode })
    }
    ...
    func _updateChildElements(
        ofType elementType: FinalCutPro.FCPXML.ElementType,
        withChild newChild: (any OFKXMLNode)?,
        default defaultChild: (any OFKXMLNode)? = nil
    ) {
        // remove existing children of the element type first
        removeChildren { child in
            child.fcpElementType == elementType
    ...
    func _updateFirstChildElement(
        ofType elementType: FinalCutPro.FCPXML.ElementType,
        withChild newChild: (any OFKXMLElement)?,
        default defaultChild: @autoclosure () -> (any OFKXMLElement)? = { nil }()
    ) {
    ...
    func _updateDefaultedFirstChildElement(
        ...
        _updateFirstChildElement(
            ofType: elementType,
            withChild: newChild,
            default: OFKXMLDefaultFactory().makeElement(name: elementType.rawValue)
        )
    }
...
extension OFKXMLElement {
    ...
    func _updateChildElement(named childName: String, newStringValue: String?) {
        if let existingChild = firstChildElement(named: childName) {
            ...
            if let newStringValue = newStringValue {
                let newNote = OFKXMLDefaultFactory().makeElement(name: childName)
...
extension OFKXMLElement {
    ...
    public func fcpAncestorClip<S: Sequence<any OFKXMLElement>>(
    ...
    ///
    /// Ancestors are ordered nearest to furthest.
    public func fcpAncestorTimeline<S: Sequence<any OFKXMLElement>>(
        ancestors: S? = nil as [any OFKXMLElement]?,
        includingSelf: Bool,
        withLaneZero: Bool = false
    ) -> (timeline: any OFKXMLElement, ancestors: AnySequence<any OFKXMLElement>)? {
    ...
    ///
    /// Ancestors are ordered nearest to furthest.
    func _fcpAncestorElementTypesAndLanes<S: Sequence<any OFKXMLElement>>(
        ancestors: S? = nil as [any OFKXMLElement]?,
        includingSelf: Bool
...
extension OFKXMLElement {
    /// FCPXML: Returns whether any descendant element matches `name`.
    func _fcpContainsDescendant(named name: String) -> Bool {
        _fcpContainsDescendant { $0.name == name }
    }
    
    /// FCPXML: Returns whether any descendant element satisfies `predicate`.
    func _fcpContainsDescendant(
        where predicate: (any OFKXMLElement) -> Bool
    ) -> Bool {
        for child in childElements {
            ...
            if child._fcpContainsDescendant(where: predicate) {
                return true
            }
        }
